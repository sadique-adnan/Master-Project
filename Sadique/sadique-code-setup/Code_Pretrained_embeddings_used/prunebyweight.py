        ...
        with tf.Session() as sess:
            ...
            def pruned_weights(variable_name):
                ...
                for i in range(num_filters):
                    weight_sum_filter = np.sum(var[:,:,:,i])
                    weight_sum.append(weight_sum_filter)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            
                weight_sum_numpy = np.asarray(weight_sum)
                weight_sum_sorted=(weight_sum_numpy.argsort())
                filters_to_prune = int(0.3*num_filters)
                #filters_to_prune = int(0.3*num_filters)
                print('filters_to_prune',filters_to_prune)
                indices = weight_sum_sorted[:filters_to_prune]
                filters_retained = num_filters - filters_to_prune
                print('filters_retained',filters_retained)
                indices_retained = weight_sum_sorted[-filters_retained:]
                print(indices)
            ...
            out_dir = os.path.abspath(os.path.join(os.path.curdir, "pruned_data_by_weight_filters_corporate_round_1", "checkpoints"))
...
def make_graph(x_train, y_train, vocab_processor, vocab_size, embedding, x_dev, y_dev):
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, tf.train.latest_checkpoint('./pruned_data_by_weight_filters_corporate_round_1/checkpoints/'))
       # print([n.name for n in tf.get_default_graph().as_graph_def().node])
        ...
        out_dir = os.path.abspath(os.path.join(os.path.curdir, "pruned_by_weight", timestamp))
        print("Writing to {}\n".format(out_dir))
...
if __name__ == '__main__':
    ...
    if args.prune:
        ...
        #filters = prune_filter_weight("./pruned_by_weight/1557092894/checkpoints/model-2600.meta", './pruned_by_weight/1557092894/checkpoints',90)
        print(filters)
